...
# All taxa had 'ids' formatted as <GenBankId_or_internal_ID>|<geotag>|<date>
# Almost dates are precise and formatted as yyyy-MM-dd, as required by Delphy
# But some dates are not precise.  For simplicity, we just suppress these samples
suppressedIds = set()
for taxon in xmlTaxa:
    taxonId = taxon.get('id')
    gbId, geo, date = taxonId.split('|')
    if len(date) != len('1970-01-01'):
        suppressedIds.add(gbId)
        print(f'Taxa {gbId} has imprecise date "{date}" -> suppressing it')
print(f'Suppressed {len(suppressedIds)} taxa with imprecise dates')
