...
if len(sys.argv) != 2:
    sys.stderr.write("Usage: prepare_runs.py <email-address-for-Entrez>\n")
    sys.exit(1)
...
def parse_genbank_date(gb_date):
    """
    Parses a date from GenBank to a Python date.
    Ambiguous date ranges are turned into a date in the range midpoint.
    
    Formats recognized from GenBank:
    * 1978
    * 09-Nov-2017
    * Nov-2017     (arbitrarily set to 2017-11-15)
    * 2019-01-16
    * 2018-08
    """
    ...
    def go(gb_date):
        """Returns (yyyy, mm, dd) tuple, anchoring to date range midpoint where needed"""
        ...
        if m := re.match(r'^(\w\w\w)-(\d\d\d\d)$', gb_date):
            if m[1].lower() in months:
                return (int(m[2]), months[m[1].lower()], 15)
        
        # 09-Nov-2017
        if m := re.match(r'^(\d\d)-(\w\w\w)-(\d\d\d\d)$', gb_date):
            if m[2].lower() in months:
                return (int(m[3]), months[m[2].lower()], int(m[1]))
        ...
        if m := re.match(r'^(\d\d\d\d)-(\d\d)$', gb_date):
            return (int(m[1]), int(m[2]), 15)
        
        # Nothing
        return (1900, 1, 1)
    
    (yyyy, mm, dd) = go(gb_date)
...
def gb_to_fasta(in_gb_xml_f, out_fasta_f, default_assembly_name='UNKNOWN'):
    ...
    for record in records:
            ...
            if feature['GBFeature_key'] == 'source':
                for qual in feature['GBFeature_quals']:
                    if qual['GBQualifier_name'] == 'collection_date':
                        ...
                        collection_date = parse_genbank_date(raw_collection_date)
        ...
        out_fasta_f.write(f'>{assembly_name}|{acc}|{collection_date.strftime("%Y-%m-%d")}\n')
...
if do_trim:
    ...
    with open(input_fasta_path, 'w') as out_f:
        for seq_id, seq in aligned_seqs:
            if seq_id.startswith(ref_acc_id):
                seq_id = f'{ref_acc_id}|2019-12-31'  # Slightly arbitrary
            else:
